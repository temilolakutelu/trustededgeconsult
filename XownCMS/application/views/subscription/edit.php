...
<div class="panel panel-default form-panel" data-widget='{"draggable": "false"}'>
    <div class="panel-heading">
        <h2>Update Subscriber Details</h2>
    </div>
    <div class="panel-body">
       
        <form action="<?= site_url('subscription/edit_action'); ?>" method="post" enctype="multipart/form-data">    
             
            <div class="form-group col-md-5">
    <input type="hidden"  name="id" value="<?= $id; ?>">
                <label for="name">Subscriber's name<span class="required">*</span></label>
                <input type="text" class="form-control" name="name" id="name" value="<?php echo $name; ?>" pattern="[a-zA-Z0-9\s]+" title="Only Alphanemeric allowed"/>
                
            </div>
            <div class="form-group col-md-5">
                <label for="email">Email</label>
                <input type="text" class="form-control" name="email" i placeholder="email" value="<?php echo $email; ?>"/>
            </div>
           
        <div class="form-group col-md-3">&nbsp;
       </div>
        <input type="submit" class="finish btn-success btn form-group col-md-3" value="Update Subscriber" />
        </form>
    </div>
</div>
