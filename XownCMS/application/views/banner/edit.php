...
<div class="panel panel-default form-panel" data-widget='{"draggable": "false"}'>
    ...
    <div class="panel-body">
        <form action="<?= site_url('banner/edit_action') ?>" method="post" enctype="multipart/form-data">            
            <div class="form-group col-sm-5">
                <label for="name">Banner name<span class="required"> *</span></label>
                <input type="text" class="form-control" name="title" id="name" value="<?= $banner_details->title; ?>" />
            </div>
             <div class="form-group col-md-5">
                ...
                <select type="text" class="region_id form-control" name="page_id" id="region_id" required>
                    <option value=""> Select a Page</option>
                     <option value="0" <?php if($banner_details->pageID == 0){ echo 'selected';} ?>> HomePage</option>
                    <?php
                        foreach ($page_list as $lists) {?>
                            <option value="<?= $lists->pageID?>" <?php if($lists->pageID==$banner_details->pageID){ echo 'selected';} ?>><?= $lists->pageName?></option>
                    <?php    }
                    ?>
                </select>
            </div>
             <input type="hidden"  name="id" value="<?= $banner_details->bannerID; ?>">
             <div class="form-group col-sm-5">
                 <label for="name">First Text</label>
                <input type="text" class="form-control" name="txt1" id="url" value="<?= $banner_details->textLine1; ?>" />
            </div>
            
             <div class="form-group col-sm-5">
                 <label for="name">Second Text </label>
                <input type="text" class="form-control" name="txt2" id="url" value="<?= $banner_details->textLine2; ?>" />
            </div>
            ...
            <img src="<?= base_url();?>images/banners/<?= $banner_details->imageURL; ?>" width="100px">
