...
<div class="panel panel-default form-panel" data-widget='{"draggable": "false"}'>
    ...
    <div class="panel-body">
        <form action="<?= site_url('video/edit_action') ?>" method="post" enctype="multipart/form-data">            
            <div class="form-group col-sm-5">
                <label for="name">Video tiltle/Description<span class="required"> *</span></label>
                <input type="text" class="form-control" name="title" id="name" value="<?= $gallery_details->title; ?>" />
            </div>
                ...
                <select type="text" class="region_id form-control" name="page_id" id="region_id" required>
                    ...
                    <?php
                        foreach ($cat_lists as $lists) {?>
                            <option value="<?= $lists->id?>" <?php if($lists->id == $gallery_details->gal_category){ echo 'selected';} ?>><?= $lists->name?></option>
                    <?php    }
                    ?>
                </select>
            </div>
             ...
             <div class="form-group col-md-10">
                ...
                <input type="text" class="form-control" name="reference" placeholder="Web reference" value="<?php echo $gallery_details->url; ?>"/>
            </div>
            ...
            <div class="col-sm-12">
                <button type="submit" class="btn btn-primary">Update Video gallery</button> 
